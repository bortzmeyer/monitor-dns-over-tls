...
main(int argc, char **argv)
{
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: %s TLS-resolver-as-IP-address domain-name\n",
                argv[0]);
    ...
    getdns_return_t dns_request_return =
        getdns_address_sync(this_context, this_server,
                            (getdns_dict *) NULL,
                            &this_response);
    ...
    getdns_return_t process_return =
        getdns_dict_get_int(this_response, "status", &this_error);
    ...
    if (process_return != GETDNS_RETURN_GOOD) {
        fprintf(stderr,
                "Trying to get the answers for the resolver's address failed: %d\n",
                process_return);
        getdns_dict_destroy(this_response);
        return (GETDNS_RETURN_GENERIC_ERROR);
    }
    ...
    /* Always zero if the argument is an IP address. So, we cheat if (num_addresses 
     * <= 0) { fprintf(stderr, "No IP addresses for the resolver\n"); return
     * (GETDNS_RETURN_GENERIC_ERROR); } getdns_dict *this_address; Test only the
     * first one process_return = getdns_list_get_dict(just_the_addresses_ptr, 0,
     * &this_address); process_return = getdns_dict_get_bindata(this_address,
     * "address_data", &this_resolver); */
    /* uint32_t localhost = htonl(0x7f000001); getdns_bindata this_resolver = { 4,
     * (void *)&localhost}; */
    getdns_dict    *this_resolver = getdns_dict_create();
    process_return = getdns_str2dict(this_server, &this_resolver);      /* str2dict
                                                                         * requires
                                                                         * getdns >= 
                                                                         * 1.1 */
    if (process_return != GETDNS_RETURN_GOOD) {
        fprintf(stderr,
                "Unable to convert %s to bindata: %d (we accept only IP addresses, not names). Exiting.\n",
                this_server, process_return);
        getdns_context_destroy(this_context);
        return (GETDNS_RETURN_GENERIC_ERROR);
    }
    ...
    /* getdns_return_t dict_set_return = getdns_dict_set_bindata(this_dict,
     * "address_type", &this_type); if (dict_set_return != GETDNS_RETURN_GOOD) {
     * fprintf(stderr, "Unable to add address type to the dict: %d. Exiting.\n",
     * dict_set_return); getdns_context_destroy(this_context); return
     * (GETDNS_RETURN_GENERIC_ERROR); } */
    /* dict_set_return = getdns_dict_set_bindata(this_dict, "address_data",
     * &this_resolver); if (dict_set_return != GETDNS_RETURN_GOOD) { fprintf(stderr, 
     * "Unable to add \"%s\" to the dict: %d. Exiting.\n", this_server,
     * dict_set_return); getdns_context_destroy(this_context); return
     * (GETDNS_RETURN_GENERIC_ERROR); } */
